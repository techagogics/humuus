...
export default function DefaultQuiz(props: any) {
  //------------------------------------------------------------
  ...
  const [question, setQuestion] = useState<defaultQuiz_Question>({
    text: '',
    options: [],
  });
  ...
  const [answer, setAnswer] = useState<Array<number>>([-1]);

  const [answered, setAnswered] = useState<boolean>(false);

  const [myAnswer, setMyAnswer] = useState<Array<number>>([-1]);

  // Keeps Up-To-Date Values for use in Event Listener
  // UseState Value is Frozen on initialized Value in initSocket()
  const upToDateAnswer = useRef<Array<number>>([-1]);
  const showingAnswer = useRef<Array<number>>([-1]);
      ...
      if (json_string !== null) {
        switch (matchState.op_code) {
          case OpCode.PLAYERLIST:
            json = JSON.parse(json_string) as defaultQuiz_PlayerListMessage;
            ...
            break;
          case OpCode.UPDATE:
            json = JSON.parse(json_string) as defaultQuiz_QuestionsMessage;
            ...
            break;
          case OpCode.ANSWER:
            json = JSON.parse(json_string) as defaultQuiz_AnswerMessage;
          ...
          case OpCode.CONFIRMED:
            json = JSON.parse(json_string) as defaultQuiz_StateMessage;
          ...
          case OpCode.TIMER:
            json = JSON.parse(json_string) as defaultQuiz_TimerMessage;
          ...
          case OpCode.END:
            json = JSON.parse(json_string) as defaultQuiz_ResultMessage;
  ...
  async function submitAnswer() {
    if (!isAnswered.current && upToDateAnswer.current[0] > -1) {
      sendAnswer();
    }
  }
        ...
        <div className="text-xl">
          ...
          {!isPresenter && (
            <Button
              ...
              size="lg"
              variant={myAnswer[0] > -1 && !answered ? 'secondary' : 'default'}
              className="m-auto mt-5"
              asChild
            >
